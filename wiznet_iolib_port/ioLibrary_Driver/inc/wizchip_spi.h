...
/**
 * @brief Check whether phy link is up or down (ON or OFF). Assumes there is a single Wizchip on the system.
 * @return 1 if link is up (ON), 0 if link is down (OFF), -1 if error.
 */
...
/*! \brief Initialize WIZchip
    ...
    Modified to return a negative integer if unsuccessful.
