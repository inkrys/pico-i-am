...
int wizchip_initialize(void) {
    ...
    // IK: Removed do while loop to externally loop in case of link down.
    // do {
...
int wizchip_check_phy_link(void)
{
    uint8_t temp = 0;

    // Would prefer to use check here, but no way to test so copying directly from
    // the original wizchip_initialize()
    if (ctlwizchip(CW_GET_PHYLINK, (void *)&temp) == -1) {
        printf(" Unknown PHY link status\n");
