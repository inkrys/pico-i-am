...
static wiz_NetInfo g_net_info = {
    ...
    .ip = {169, 254, 164, 3},                    // IP address
    .sn = {255, 255, 255, 0},                    // Subnet Mask
    .gw = {169, 254, 164, 1},                     // Gateway
...
static uint8_t g_mqtt_broker_ip[4] = {169, 254, 164, 2};
...
int main() {
    ...
    check(pico_i_am_setup_wXXXX_evb_pico() == 0, "Board setup failed.");

    struct pico_i_am_ConnData *conn_data = pico_i_am_init_conn(g_mqtt_network, SOCKET_MQTT);
    check(conn_data != NULL, "Failed to initialize network connection conn.");
    // NewNetwork(&g_mqtt_network, SOCKET_MQTT); // This just sets the memebers of the network structure with the wiz chip callbacks and the socket.
    ...
    //     log_info("Network connection failed at t + %d ms.", timestamp_ms);
    //     sleep_ms(DEFAULT_TIMEOUT);
    // }
...
error:
    log_err("Critical fault, going into trap.");
    while(1) {
        log_err("Trapped because of critical fault; sending this message every 5s.");
        sleep_ms(5000);
    }    
}
